...
decompose.1d.ssa.svd <- function(x,
  ...
  # Build hankel matrix
  if (is.null(x$wmask) && is.null(x$fmask) && is.null(x$weights)) {
    h <- hankel(.F(x), L = L)
  } else {
...
.get.or.create.Lcov.matrix <- function(x)
  .get.or.create(x, "Lcov.matrix", .Lcov.matrix(x))
...
decompose.1d.ssa.eigen <- function(x,
  ...
  # Do decomposition
  S <- eigen(.get.or.create.Lcov.matrix(x), symmetric = TRUE)
