...
.get.orth.triples <- function(x, idx, tolerance = sqrt(.Machine$double.eps)) {
  ...
  if (min(sigma) < tolerance)
    warning("Decomposition isn't minimal. Some singular values equal to zero")

  list(sigma = sigma, U = U, V = V)
}

iossa <- function(x, groups, ..., tol = 1e-5, kappa = 1.2,
                  maxiter = 1000,
                  initial.approx = reconstruct(x, groups),
                  norm = function(x) {
                    sqrt(mean(x^2))
                  }, trace = FALSE,
                  kappa.balance = 0.5) {
  ...
  groups <- lapply(groups, function(group) match(group, idx))
  ...
  .set(x, "iossa.groups",  groups)
  .set(x, "iossa.groups.all", c(groups, iossa.groups.all[valid.groups]))
...
genwcor <- function(x, groups, basis, ..., cache = TRUE) {
  ...
  if (missing(basis)) {
    basis <- unlist(.get(x, "iossa.groups.all"))
    if (is.null(basis)) {
