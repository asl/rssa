...
.nspecial.pssa <- function(x) {
  ...
  decomposition$nPR + decomposition$nPL
}
...
decompose.pssa.nutrlan <- function(x,
  ...
  S <- trlan.svd(ph, neig = neig, ...,
                 lambda = sigma[-seq_len(nspecial)], U = U[, -seq_len(nspecial)])
...
rforecast.pssa <- function(x, groups, len = 1,
  ...
  for (i in seq_along(groups)) {
    ...
    # Calculate drifts
    sigma <- .sigma(x)[right.group]
    U <- .U(x)[, right.group, drop = FALSE]
    V <- calc.v(x, right.group)
...
vforecast.pssa <- function(x, groups, len = 1,
  ...
  for (i in seq_along(groups)) {
    ...
    Z <- V * rep(.sigma(x)[right.group], each = nrow(V))

    U.head.nr <- .U(x)[-L, nonright.group, drop = FALSE]
    P.head <- tcrossprod(qr.Q(qr(U.head.nr)))

    X <- .U(x)[, group, drop = FALSE] %*% (.sigma(x)[group] * t(calc.v(x, group)))
    C <- U %*% t(Z)
