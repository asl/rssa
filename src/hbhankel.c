...
static void hbhankel_xmatmul(double* out,
                             const double* v,
                             const void* matrix,
                             int transposed) {
  const hbhankel_matrix *h = matrix;
...
static void hbhankel_matmul(double* out,
                            const double* v,
                            const void* matrix) {
  hbhankel_xmatmul(out, v, matrix, 0);
}

static void hbhankel_tmatmul(double* out,
                             const double* v,
                             const void* matrix) {
  hbhankel_xmatmul(out, v, matrix, 1);
}
...
SEXP hbhmatmul(SEXP hmat, SEXP v, SEXP transposed) {
  ...
  if (LOGICAL(tchk)[0]) {
    ...
    hbhankel_xmatmul(REAL(Y), REAL(v), h, LOGICAL(transposed)[0]);
